...
2. **Edición mínima: cambiá SOLO las palabras objetivo.** Las palabras objetivo son la UNIÓN de `misplacedLemmas` y `outOfCatalogWords`. Partí de la oración ORIGINAL y reemplazá ÚNICAMENTE esas palabras; conservá TODO el resto tal cual — el sujeto, el objeto, la estructura y las demás content-words (p.ej. bicycle, mountains, stories). NO reescribas la oración de cero ni cambies el sujeto/contexto "porque sí". Cada content-word que dejás en su lugar aporta cobertura de vocabulario del curso; reescribir de cero esa cobertura la DESTRUYE y hunde las métricas agregadas del curso. **Preservá el OBJETO/complemento: si la palabra objetivo es un verbo, elegí un reemplazo que COLOCACIONE con el MISMO objeto** (p.ej. «The adults **consume** a lot of sugar» → «The adults **eat** a lot of sugar», porque *eat* va con *sugar*; NUNCA «The adults shower», que tira «a lot of sugar»). **Para una palabra de `outOfCatalogWords` la corrección preferida es:** (1) si es un **error ortográfico evidente** de una palabra del nivel, corregí la ortografía — SOLO si la palabra corregida es del nivel; (2) si es una palabra **genuina pero avanzada/rara**, reemplazala por un equivalente del nivel, con preferencia por `{curatedWords}`/`{suggestedWords}` cuando colocacionen. En ambos casos rigen las MISMAS restricciones de colocación y de preservación del objeto/complemento que para `misplacedLemmas`. **La edición mínima MANDA sobre usar una palabra de `{curatedWords}`:** las palabras curadas son una PREFERENCIA solo cuando encajan con el objeto/contexto original SIN reescribir. Si NINGUNA palabra curada colocaciona con el objeto original, usá una palabra del nivel que SÍ encaje y conservá el resto — está PROHIBIDO cambiar el objeto/sujeto/contexto para forzar una palabra curada. Las ÚNICAS razones válidas para cambiar algo más que las palabras objetivo son: (a) que tu edición mínima colisione con un hermano (regla #5), (b) un punto explícito del "Feedback del intento anterior", o (c) que la oración quede FUERA del rango de longitud del nivel y haya que meterla en rango (regla #6) — y en ese caso, con el cambio MÁS chico posible (sacá/agregá lo opcional, no reescribas).
...
- **Edición mínima (regla #2):** partí de la oración ORIGINAL y tocá SOLO las palabras objetivo — la UNIÓN de `misplacedLemmas` y `outOfCatalogWords`. El resto de la oración queda EXACTAMENTE igual (mismo sujeto, objeto, estructura y demás content-words). No cambies sujeto/contexto salvo colisión con un hermano (regla #5) o un punto explícito del feedback.
...
- Para cada palabra de `outOfCatalogWords`: si es un typo evidente de una palabra del nivel, corregí la ortografía (solo si la corregida es del nivel); si es una palabra genuina pero avanzada/rara, reemplazala por un equivalente del nivel — preferí `{curatedWords}`/`{suggestedWords}` cuando colocacionen con el objeto/contexto original.
...
- **Edición mínima:** cambiaste SOLO las palabras objetivo (`misplacedLemmas` ∪ `outOfCatalogWords`); el resto de la oración (todas las content-words que no eran objetivo) quedó intacto respecto del original — salvo que hubiera colisión con un hermano (regla #5).
...
- palabras fuera de catálogo (typo evidente → corregí la ortografía si es del nivel; genuina avanzada/rara → reemplazá por un equivalente del nivel): {outOfCatalogWords}
