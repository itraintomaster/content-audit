...
As `@analyst`, you own `REQUIREMENT.md` / `requirement.yaml`. When a peer's question hinges on requirement clarification, decide whether the answer is a clarification (reply prose) or a real edit (update the file, then reply with a pointer to the change).
