...
### Journey[F-LREC-J002] - Identificar lemas sub-expuestos que el estudiante podria olvidar
**Validation**: AUTO_VALIDATED

```yaml
journeys:
  - id: F-LREC-J002
    name: Identificar lemas sub-expuestos que el estudiante podria olvidar
    flow:
      - id: revisar_puntuacion
        action: "El usuario revisa el resultado de la auditoria de recurrencia y observa que la puntuacion general es baja (por ejemplo, 0.65)"
        then: consultar_resumen

      - id: consultar_resumen
        action: "El usuario consulta el resumen de exposicion y observa una cantidad significativa de lemas sub-expuestos (por ejemplo, 350 de 2000)"
        outcomes:
          - when: "Hay lemas sub-expuestos que investigar"
            then: filtrar_sub_expuestos
          - when: "No hay lemas sub-expuestos, la puntuacion baja se debe a sobre-exposicion"
            then: sin_sub_expuestos

      - id: filtrar_sub_expuestos
        action: "El usuario filtra las estadisticas por lema para ver solo los lemas con estado sub-exposed"
        then: identificar_lemas_criticos

      - id: identificar_lemas_criticos
        action: "El usuario identifica lemas importantes para el nivel del curso que aparecen muy esporadicamente (por ejemplo, 'achieve' con intervalo medio de 2500 palabras)"
        then: revisar_posiciones

      - id: revisar_posiciones
        action: "El usuario revisa las posiciones de aparicion del lema y evalua su distribucion a lo largo del curso"
        outcomes:
          - when: "El lema esta concentrado en una unica seccion del curso"
            then: decidir_redistribuir
          - when: "El lema tiene pocas apariciones repartidas pero con brechas muy grandes"
            then: decidir_agregar_apariciones

      - id: decidir_redistribuir
        action: "El usuario decide redistribuir el lema agregando oraciones que lo utilicen en otras secciones del curso"
        result: success

      - id: decidir_agregar_apariciones
        action: "El usuario decide agregar mas oraciones con este lema en las zonas del curso donde esta ausente"
        result: success

      - id: sin_sub_expuestos
        action: "El usuario concluye que no hay lemas sub-expuestos y redirige su atencion a otros aspectos del resultado"
        result: success
```

### Journey[F-LREC-J003] - Identificar lemas sobre-expuestos que saturan al estudiante
**Validation**: AUTO_VALIDATED

```yaml
journeys:
  - id: F-LREC-J003
    name: Identificar lemas sobre-expuestos que saturan al estudiante
    flow:
      - id: revisar_resultado
        action: "El usuario revisa el resultado de la auditoria de recurrencia y consulta las estadisticas por lema"
        then: filtrar_sobre_expuestos

      - id: filtrar_sobre_expuestos
        action: "El usuario filtra los lemas con estado over-exposed e identifica lemas que aparecen con demasiada frecuencia (por ejemplo, 'make' con intervalo medio de 15 palabras)"
        outcomes:
          - when: "Hay lemas sobre-expuestos que investigar"
            then: evaluar_causa
          - when: "No hay lemas sobre-expuestos"
            then: sin_sobre_expuestos

      - id: evaluar_causa
        action: "El usuario evalua si la sobre-exposicion es inherente al contenido del curso (palabras naturalmente frecuentes como 'make' o 'get') o si indica un problema de variedad lexica"
        outcomes:
          - when: "La sobre-exposicion es inherente al idioma y al nivel del curso"
            then: aceptar_sobre_exposicion
          - when: "La sobre-exposicion indica falta de variedad lexica"
            then: buscar_alternativas

      - id: buscar_alternativas
        action: "El usuario busca sinonimos o reformulaciones para distribuir mejor el vocabulario y reducir la repeticion excesiva del lema"
        result: success

      - id: aceptar_sobre_exposicion
        action: "El usuario acepta la sobre-exposicion como caracteristica natural del contenido y no realiza cambios para ese lema"
        result: success

      - id: sin_sobre_expuestos
        action: "El usuario concluye que no hay lemas sobre-expuestos y redirige su atencion a otros aspectos del resultado"
        result: success
```
...
### Journey[F-LREC-J005] - Comparar la recurrencia antes y despues de ajustar el contenido
**Validation**: AUTO_VALIDATED

```yaml
journeys:
  - id: F-LREC-J005
    name: Comparar la recurrencia antes y despues de ajustar el contenido
    flow:
      - id: modificar_contenido
        action: "El usuario modifica el contenido del curso para mejorar la distribucion de los lemas problematicos identificados en auditorias previas"
        then: reejecutar_auditoria

      - id: reejecutar_auditoria
        action: "El usuario re-ejecuta la auditoria de recurrencia sobre el curso modificado"
        then: comparar_puntuacion

      - id: comparar_puntuacion
        action: "El usuario compara la puntuacion general antes y despues del ajuste (por ejemplo, de 0.65 a 0.78)"
        outcomes:
          - when: "La puntuacion mejoro significativamente"
            then: verificar_lemas_ajustados
          - when: "La puntuacion no mejoro o empeoro"
            then: analizar_regresion

      - id: verificar_lemas_ajustados
        action: "El usuario verifica que los lemas especificamente ajustados han mejorado su clasificacion de exposicion"
        outcomes:
          - when: "Los lemas ajustados ahora tienen exposicion normal y la mejora es suficiente"
            then: mejora_exitosa
          - when: "Algunos lemas mejoraron pero otros siguen siendo problematicos"
            then: iterar_mejora

      - id: analizar_regresion
        action: "El usuario investiga por que la puntuacion no mejoro, verificando si los cambios introdujeron nuevos problemas de recurrencia en otros lemas"
        then: iterar_mejora

      - id: iterar_mejora
        action: "El usuario repite el proceso de ajuste enfocandose en los lemas que aun son problematicos, volviendo a modificar el contenido"
        result: success

      - id: mejora_exitosa
        action: "El usuario confirma que la distribucion de vocabulario del curso ha alcanzado un nivel satisfactorio de recurrencia"
        result: success
```
