...
2026-07-30 — qa-tester — Hueco de cobertura del titulo en la correccion lexica: 1 handwrittenTest nuevo sobre DefaultLemmaAbsenceProposalDeriver tagueado F-LAPS-R014 ("elementAfter conserva el titulo de elementBefore"). Patch propuesto en requirements/2026-04-20.02_.../architectural_patch.yaml (0 adds, 1 mod); el guard `--as=qa-tester` lo rechaza por el anidado en packages[engine] — lo aplica el usuario sin `--as`.
  why: el deriver escribia la oracion plana derivada tambien en `title`, y con el guard de preservacion de FEAT-RPRES toda aprobacion lexica devolvia PRESERVATION_VIOLATED. Ninguno de los 3 tests vecinos lo atrapaba: el test de R014 vigente enumera quizId/nodeIds/instructions/order pero NO el titulo; DefaultCorrectionScopeTest verifica el alcance declarado, no que el deriver lo respete; DefaultProposalDecisionServiceTest usa propuestas armadas a mano.
  tag: R014 y no F-RPRES-R004 — R014 es la regla que constrine literalmente el diff elementBefore/elementAfter de la correccion lexica y nombra el titulo como ejemplo de lo preservado; R004 gobierna la propagacion etiqueta->titulo a nivel sistema (tras aplicar y recargar), que el deriver no ejerce.
  ojo: DefaultLemmaAbsenceProposalDeriverTest YA tiene un @Tag("F-LAPS-R014"), asi que `sentinel generate` no materializa el stub nuevo (bug conocido de hasMatchingTaggedTest en GenerateCommand). Hace falta el workaround de copiar el stub del artifact fresco.

2026-07-30 — test-writer — Testigo obsoleto corregido en FLapsJ001/J002 (x2)/J005JourneyTest: 5 aserciones usaban `elementAfterQuiz.getTitle()` para verificar la oracion plana derivada (R001/R012), pero desde el fix de produccion el titulo se preserva del elementBefore (F-LAPS-R014/F-RPRES-R004) y la oracion plana vive solo en `sentences`. Movido el testigo a `getSentences()` (variante canonica = `List.of(...)`, indice 0) y agregada en cada caso la asercion complementaria de que `title` quedo preservado igual al del elementBefore.
  why: el testigo viejo no solo apuntaba al campo equivocado — ademas exigia (via assertNotEquals) que el titulo cambiara, es decir, validaba el comportamiento incorrecto que causo el bug. La complementaria de preservacion es la mitad que faltaba para que el test hubiera atrapado la regresion.

2026-07-30 — developer — DefaultLemmaAbsenceProposalDeriver: `parse` -> `parseOnto(candidate.getQuizSentence(), baseForm)`, con fallback `baseForm = beforeQuiz.getForm() != null ? beforeQuiz.getForm() : new FormEntity()` para el caso sin form previo.
  why: `parse()` fabrica el FormEntity desde cero y pierde kind/incidence/label/name + vacia (a null) el texto de cada CLOZE; `parseOnto` (course-domain, ya implementado y probado 50/50) preserva esos atributos del before y solo reemplaza lo que el DSL codifica. Pasar un `new FormEntity()` como base cuando no hay form evita reintroducir el bug (parseOnto sigue garantizando texto CLOZE "" en vez de null) sin necesitar una rama especial de manejo de errores.
