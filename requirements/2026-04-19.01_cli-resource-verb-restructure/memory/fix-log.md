...
2026-04-19 — developer — `sentinel patch propose` for GetCmd.correctionContextResolver merged into existing architectural_patch.yaml (which already had AnalyzeCmd handwrittenTest). The merged YAML put AnalyzeCmd at root-level implementations (duplicate of the one in the commands package). Fix: manually removed the root-level `implementations: [{name: AnalyzeCmd, ...}]` block from sentinel.yaml before regenerating.
  why: `sentinel patch propose` merges by appending; when the existing patch had AnalyzeCmd at root-level and the real definition is in commands package, the result had two AnalyzeCmd entries causing a name-conflict in `sentinel generate`.
2026-04-19 — developer — GetCmdTest.setUp uses 3-arg GetCmd constructor; adding the 4th parameter broke 20 tests. Fixed by adding `@Mock CorrectionContextResolver correctionContextResolver` and passing it as the 4th arg in setUp. This is a developer-maintained section (written by @test-writer); Sentinel smart merge preserves it, so edit it directly.
  why: sentinel generate sees the test file as "up to date" (smart-merge) — it won't auto-update the constructor call in setUp.

2026-04-19 — qa-tester — Re-declaration of correction-context tests lost by CLIRV cannot live in FEAT-CLIRV's patch folder. The validator cross-references each handwrittenTest's traceability.feature against the patch's `code:`; cross-feature handwrittenTests are rejected. Solution: write two requirement-scoped patches in the FEAT-RCSL and FEAT-RCLA folders (architectural_patch.yaml in each). FEAT-CLIRV's own patch file restored to its committed state (AnalyzeCmd handwrittenTest only).
  why: per `sentinel patch propose` schema, a patch's `code:` must equal every test traceability.feature in the patch — multi-feature patches are not valid even when the destination folder is shared.
