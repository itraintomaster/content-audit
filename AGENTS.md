...
6. **Development** — Implement the contracts and the test stubs generated by Sentinel. For `handwrittenTests`: the test class and method stubs already exist in `src/test/`. Implement the method bodies without renaming methods or creating new test files. Method names are canonical — derived from the test name in `sentinel.yaml`.
